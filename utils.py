        ...
        for cls in range(len(class_names)):
            ...
            if np.size(boxes) != 0:
                for box in boxes:
                    ...
                    cv.rectangle(img, (xy[0], xy[1]), (xy[2], xy[3]), color[::-1], 2)
                    (test_width, text_height), baseline = cv.getTextSize(class_names[cls]+confidence,
                                                                          cv.FONT_HERSHEY_SIMPLEX,
                                                                          0.75, 1)
                    cv.rectangle(img, (xy[0], xy[1]),
                                  (xy[0] + test_width, xy[1] - text_height - baseline),
                                  color[::-1], thickness=cv.FILLED)
                    cv.putText(img, class_names[cls]+confidence, (xy[0], xy[1] - baseline),
    ...
    for cls in range(len(class_names)):
        ...
        if np.size(boxes) != 0:
            for box in boxes:
                ...
                #confidence = ' '+str(confidence*100)[:2]
                xy = [int(xy[i] * resize_factor[i % 2]) for i in range(4)]
                cv.rectangle(frame, (xy[0], xy[1]), (xy[2], xy[3]), color[::-1], 2)
                (test_width, text_height), baseline = cv.getTextSize(class_names[cls]+confidence,
                                                                      cv.FONT_HERSHEY_SIMPLEX,
                                                                      0.75, 1)
                cv.rectangle(frame, (xy[0], xy[1]),
                              (xy[0] + test_width, xy[1] - text_height - baseline),
                              color[::-1], thickness=cv.FILLED)
                cv.putText(frame, class_names[cls]+confidence, (xy[0], xy[1] - baseline),
                            cv.FONT_HERSHEY_SIMPLEX, 0.75, (0, 0, 0), 1)
